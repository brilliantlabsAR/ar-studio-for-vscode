...
export function activate(context: vscode.ExtensionContext) {
  ...
  const nodeDependenciesProvider = new DepNodeProvider("rootPath");
    ...
    //   // Subscribe to the onDragData event
	//   nodeDependenciesProvider.onDragData((data: any) => {
	...
	// 	//   if (panel) {
	// 	// 	// Pass the drag data to the webview
	// 	// 	//panel.webview.postMessage({ command: 'dragData', data });
	// 	//   }
	//   }),
  ...
  if (!initializedWorkSpace) {
    vscode.workspace.updateWorkspaceFolders(
      vscode.workspace.workspaceFolders
        ...
        : 0,
      null,
      { uri: vscode.Uri.parse(myscheme + ":/"), name: myscheme }
    );
  ...
  // 	vscode.window.registerWebviewViewProvider(MyViewProvider.viewType, new MyViewProvider(context.extensionUri, myProvider))
  //   );
  ...
  const alldisposables = vscode.Disposable.from(
    ...
    vscode.workspace.registerFileSystemProvider(myscheme, memFs, {
    ...
    vscode.languages.registerDocumentDropEditProvider("python", {
		...
		provideDocumentDropEdits(document: vscode.TextDocument, position: vscode.Position, dataTransfer: vscode.DataTransfer, token: vscode.CancellationToken) {
			let itemValue:any;
			dataTransfer.forEach(item => {
				if(item && item.value){
					let id:string='';
					id=(JSON.parse(item.value).itemHandles[0]).replace('1/','')
							for (let key in snippets.display) {
								if (snippets.display.hasOwnProperty(key)) {
									let innerObj = snippets.display[key];
									if (innerObj["id"] == id) {
										itemValue= innerObj.body;			
										break;
									}
								}
							}
       ...
       //  new vscode.SnippetTextEdit(new vscode.Position(0, 0),itemValue)
        return new vscode.DocumentDropEdit(itemValue);
      },
    }),
    ...
    vscode.commands.registerCommand("brilliant-ar-studio.helloWorld", () => {
      ...
      vscode.window.showInformationMessage(
        "Hello World from Brilliant AR Studio!"
      );
    }),
