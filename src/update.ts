...
export async function checkForUpdatess() {
    try {
        await replRawMode(true);
    // Short delay to throw away bluetooth data received upon connection
    await new Promise(r => setTimeout(r, 100));
...
export async function startFpgaUpdatee(binPath?:vscode.Uri) {
    ...
    for (let chk = 0; chk < chunks; chk++) {
        let response:any = await replSend("update.Fpga.write(ubinascii.a2b_base64(b'" +
