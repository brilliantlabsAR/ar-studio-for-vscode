...
export class ProjectProvider
  implements
    vscode.TreeDataProvider<Project>,
    vscode.TreeDragAndDropController<Project>
{
  dropMimeTypes = [];
  ...
  public async handleDrag(
    ...
    treeDataTransfer.set(
      ...
      new vscode.DataTransferItem(source[0].label)
    );
    return;
  }
...
export class GitOperation {
  ...
  async init() {
    this.auth = await vscode.authentication.getSession("github", this.scopes, {
      createIfNone: true,
    });
  }
  getOwnerRepo(pushUrl: string) {
    let ownerRepo = pushUrl
...
export function cloneAndOpenRepo(repoUrl: string, uri: vscode.Uri): void {
  ...
  gitClone.on("exit", () => {
    ...
    vscode.workspace.updateWorkspaceFolders(workspaceFolder, 0, {
      uri,
      name: folderName,
    });
  });
